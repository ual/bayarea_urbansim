import numpy as np
import pandas as pd
from asim_utils import quick_loc_series
...
class OffsetMapper(object):
    ...
    def set_offset_list(self, offset_list):
        ...
        if (offset_list == range(
                first_offset, len(offset_list) + first_offset)):
            offset_int = -1 * first_offset
            self.set_offset_int(offset_int)
            return

        if self.offset_series is None:
            self.offset_series = pd.Series(
                data=range(len(offset_list)), index=offset_list)
        else:
    ...
    def map(self, zone_ids):
        ...
        if self.offset_series is not None:
            ...
            offsets = np.asanyarray(
                quick_loc_series(zone_ids, self.offset_series))
...
class SkimDict(object):
    ...
    Note that keys are either strings or tuples of two strings
    to support stacking of skims.
    """
...
class SkimDictWrapper(object):
    """
    A SkimDictWrapper object is an access wrapper around a SkimDict of
    multiple skim objects, where each object is identified by a key.  It
    operates like a dictionary - i.e. use brackets to add and get skim
    objects - but also has information on how to lookup against the skim
    objects. Specifically, this object has a dataframe, a left_key and
    right_key. It is assumed that left_key and right_key identify columns
    in df. The parameter df is usually set by the simulation itself as
    it's a result of interacting choosers and alternatives.
...
class SkimStack(object):
    ...
    def __init__(self, skim_dict):
        ...
        for key, skim_data in skim_dict.skims.iteritems():
            if not isinstance(key, tuple) or not len(key) == 2:
                print("SkimStack __init__ skipping key: %s" % key)
                continue
            print("SkimStack __init__ loading key: %s" % (key,))
        ...
        for skim_key1, value in self.skims_data.iteritems():
            ...
            self.skim_keys_to_indexes[skim_key1] = dict(
                zip(value.keys(), range(len(value))))

        print("SkimStack.__init__ loaded %s keys with %s total skims" % (
    ...
    def wrap(self, left_key, right_key, skim_key):
        ...
        return SkimStackWrapper(stack=self,
                                left_key=left_key,
                                right_key=right_key, skim_key=skim_key)


class SkimStackWrapper(object):
    """
    A SkimStackWrapper object wraps a skims object to add an additional
    wrinkle of lookup functionality.  Upon init the separate skims objects
    are processed into a 3D matrix so that lookup of the different skims can
    ...
    right_key from the Skims object and skim_key from this one, are then used
    to dereference the 3D matrix.  The tricky part comes in defining the key
    which matches the 3rd dimension of the matrix, and the key which is passed
    into __getitem__ below (i.e. the one used in the specs).  By convention,
